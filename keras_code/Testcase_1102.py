...
# 导入简化后的原始数据
newData1=pd.read_csv("./4_output_Simple.csv")
...
def generator(data,
    ...
    while 1:
        ...
        yield samples,to_categorical(targets,num_classes=4)
...
model1.add(layers.Bidirectional(layers.LSTM(32,activation="relu",return_sequences=True)))
model1.add(layers.Bidirectional(layers.LSTM(32,activation="relu",return_sequences=True)))
model1.add(layers.Bidirectional(layers.LSTM(32,activation="relu",return_sequences=True)))
